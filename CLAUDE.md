...
## 18. Custom Extensions (ISC)

Project-specific agents, skills, and commands added on top of the MoAI catalog. Custom items take priority for ISC workflows; MoAI items remain available for standard `/moai` pipelines.

### Custom Agents (`.claude/agents/` root)

| Agent | Use For | MoAI Fallback |
|---|---|---|
| `legacy-system-analyst` | Analyze existing/legacy codebase BEFORE migrate, refactor, or add features. MUST run first when source code already exists and task involves understanding it | (none) |
| `git-manager` | ISC-flavored git workflow (commits, branches, PRs) | `manager-git` |
| `docs-manager` | ISC-flavored docs (README, API docs, technical writing) | `manager-docs` |
| `code-reviewer` | Practical code review with edge-case scout + verify completion claims before PRs | `manager-quality`, `evaluator-active` |

Routing precedence:
- Direct user mention (e.g. "use code-reviewer") → custom agent
- `/moai` subcommands (plan/run/sync/etc.) → MoAI agents (manager-*, expert-*)
- Legacy/migration/reverse-engineering intent → `legacy-system-analyst` first, then MoAI chain
- Never invoke both custom and MoAI equivalents for the same task — pick one

### Custom Skills (short names)

| Skill | Use For | MoAI Overlap |
|---|---|---|
| `databases` | Direct SQL/NoSQL design, queries, psql CLI | `moai-domain-database` |
| `backend-development` | API/auth/microservices implementation | `moai-domain-backend` |
| `debug` | Root-cause analysis, multi-layer validation | `expert-debug` |
| `docs` | Init/update/summarize project docs | `moai-docs-generation` |
| `web-testing` | Playwright/Vitest/k6 E2E + load + a11y | `moai-workflow-testing` |
| `git` | Conventional commits with auto-split + secret scan | `moai-ref-git-workflow` |
| `code-review` | Pre-PR review, completion verification | `manager-quality` |

Skill precedence:
- User invokes by short name (e.g. `Skill("debug")`) → custom skill
- Inside `/moai` workflow chain → MoAI skills (`moai-*`)
- Never load both for the same task

### Custom Commands

| Command | Purpose |
|---|---|
| `/isc-review` | Review staged code against ISC Technical Standards + Git Workflow |
| `/isc-review-be` | Backend-specific ISC review with security + performance focus |
| `/test ui` | Run UI tests on a website, generate detailed report |
| `/review codebase` | Scan and analyze entire codebase |
| `/docs init` | Analyze codebase, create initial documentation |
| `/docs update` | Update existing documentation from code |
| `/docs summarize` | Summarize current documentation |

### Selection Decision (Extension)

Extends Section 4 Selection Decision Tree:

1. Existing codebase + user wants to understand/migrate/refactor? → `legacy-system-analyst` FIRST
2. ISC-specific git/docs/review task? → custom agent (git-manager, docs-manager, code-reviewer)
3. Direct skill invocation by user (short name)? → custom skill
4. Otherwise → fall through to Section 4 MoAI catalog

---

## 19. TL Role Workflows

Tech Lead playbooks for human-facing planning phases. Upstream of MoAI SPEC pipeline. Each workflow gates between phases via AskUserQuestion and produces typed artifacts in the project root.

For full design principles, extension guide, and planned workflows, see `.claude/rules/workflows/tl-roles.md`.

### Active TL Workflows

| Workflow | Command | Skill | When to Use |
|---|---|---|---|
| New Feature on Existing Codebase | `/tl new-feature [name]` | `tl-new-feature` | Add a feature to a running system with existing code, modules, and patterns. 6 phases: legacy analysis → requirements → system design → software design → impact analysis → user stories |
| Greenfield Project | `/tl greenfield [name]` | `tl-greenfield-project` | Plan a brand-new system from scratch (no existing code). 5 phases: requirements → system design + ADRs → software design → analysis + project CLAUDE.md → sprint-based user stories. Sprint 0 Foundation is mandatory. |

### Routing Rules

- "thêm tính năng mới", "new feature on existing system" → auto-trigger `tl-new-feature`
- "dự án mới", "greenfield", "new project from scratch" → auto-trigger `tl-greenfield-project`
- Existing codebase without docs (no SRS / BRD / architecture) → `tl-new-feature` Step 0 chains `legacy-system-analyst` automatically
- Empty target directory + BA docs for new system → `/tl greenfield`
- TL Step 5 (user-stories.md) → recommended handoff to `/moai plan` for SPEC EARS conversion. Greenfield additionally chains `/moai project` for scaffolding.

### Scenario Selection Heuristic

| Signal | Route |
|---|---|
| Existing code or CLAUDE.md at project root | `/tl new-feature` |
| Empty directory or fresh repo, no codebase | `/tl greenfield` |
| Migration from legacy to new system | `/tl new-feature` (Step 0 legacy analysis first) |
| Net-new system, no constraints from existing code | `/tl greenfield` |

### Integration with MoAI Pipeline

```
TL Workflow (planning, human-facing)     MoAI Pipeline (execution)
────────────────────────────────         ─────────────────────────
/tl new-feature
  Step 0 → legacy-system-analyst
  Step 1-5 → planning artifacts    ───►  /moai plan SPEC-XXX
                                         /moai run SPEC-XXX (DDD/TDD)
                                         /moai sync SPEC-XXX (docs+PR)
```

TL workflow is NOT a replacement for `/moai plan` — it is the upstream planning layer that feeds structured inputs into the MoAI SPEC pipeline.

### Extension

To add a new TL workflow (e.g. `/tl bugfix`, `/tl refactor`):
1. Create `.claude/skills/tl-<scenario>/SKILL.md`
2. Create `.claude/commands/tl/<scenario>.md` (thin wrapper)
3. Add templates in `.claude/templates/tl/<scenario>/`
4. Register in `.claude/rules/workflows/tl-roles.md` and this section

---

Version: 14.3.0 (Agency v3.2 + Harness Design Integration + ISC Extensions + TL Workflows: new-feature, greenfield)
